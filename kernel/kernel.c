...
void heap_manager_helper(uintptr_t previous_break, uintptr_t new_program_break){
    ...
    {
        // Unmap pages that no longer overlap with the heap
        // Ensure previous_break is page-aligned
        ...
        // Unmap pages till new_program_break
        for (uintptr_t addr = page_aligned_addr; addr >= new_program_break; addr-=PAGESIZE)
        {
            ...
            if (map.pn != -1)
            {
                ...
                // Decrease page refcount if process has access to it
                if (map.perm & PTE_P)
                {
                    pageinfo[map.pn].refcount -= 1;
                }
                // Set page as FREE if no other process references it
        ...
        case INT_SYS_SBRK:
            {
                ...
                // Move program break to new location
                else
                {
